...
export const testeGetAllJson = async (req: Request, res: Response): Promise<void> => {
  try {
    const filePath = path.resolve('./src/samples/teste.json');
...
export const testAddJson = async (req: Request, res: Response): Promise<void> => {
  try {
    const filePath = path.resolve('./src/samples/teste.json');
    const fileContent = fs.readFileSync(filePath, 'utf-8');
    const data = JSON.parse(fileContent);
...
export const testeUpdateJson = async (req: Request, res: Response): Promise<void> => {
  try {
    const filePath = path.resolve('./src/samples/teste.json');
    const fileContent = fs.readFileSync(filePath, 'utf-8');
    const data = JSON.parse(fileContent);
...
export const testeDeleteJson = async(req: Request, res: Response): Promise<void> => {
  try {
    const filePath = path.resolve('./src/samples/teste.json');
    const fileContent = fs.readFileSync(filePath, 'utf-8');
    ...
    console.log("Request params:", req.params); // Adiciona log para verificar os parâmetros da requisição
